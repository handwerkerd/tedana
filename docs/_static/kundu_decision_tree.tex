...
% ----- node: input
\node(input) [io,label={90:\textbf{Kundu Decision Tree (Tedana implementation)}}] {Components with calculated metrics};
% ----- node: 0
...
\node(rej4)[accept, right of=9, xshift=3cm]{Provisional accept};
% ----- node: 10
\node(10)[decision, below of=9,label={180:$node\ 10$}, yshift=-3.5cm]{$\rho > \rho$ elbow  };
\node(rej5)[reject, right of=10, xshift=3cm]{Unclassified};
% ----- node: 11
\node(11)[decision, below of=10, label={180:$node\ 11$}, yshift=-3.5cm]{ \textit{n} classified as $Provisional\ accept < 2$};
\node(rej6)[accept, right of=11, xshift=3cm]{Rerun ICA, metric calcs, \& component selection. If max restarts reached, accept everything not already rejected};
% ----- node: 12
...
\node(rej7)[reject, right of=17, xshift=4cm]{Less likely BOLD $\rightarrow$ Reject};
% ----- node: 18
...
\node(rej8)[accept, right of=18, xshift=4cm]{Low variance $\rightarrow$ Accept};
% ----- node: 19
\node(19)[decision, below of=18,label={180:$node\ 19$},label={315: Nothing unclassified remains}, yshift=-4.5cm]{\textit{n} Unclassified $==0$};
\node(rej9)[accept, right of=19, xshift=3cm]{Provisional accept$\rightarrow$ Accept};
% ----- node: 20
...
\node(rej10)[reject, right of=21, xshift=4cm]{Less likely BOLD$\rightarrow$ Reject};
% ----- node: 22
...
\node(rej11)[reject, right of=22, xshift=4cm]{Less likely BOLD$\rightarrow$ Reject};
% ----- node: 23
...
\node(rej12)[accept, right of= 24, xshift=4cm]{Accept borderline};
% ----- node: 25
\node(25)[decision, below of=24,label={180:$node\ 25$}, yshift=-4cm]{ $\kappa$ $>$ $\kappa$ elbow \& varex $>$ new lower varex};
\node(rej13)[accept, right of=25, xshift=3cm]{Accept borderline};
% ----- node: 26
\node(26)[accept, below of=25,label={180:$node\ 26$}, yshift=-3cm]{Remaining Unclassified \& Provisional accept $\rightarrow$ Likely BOLD $\rightarrow$ Accept};
